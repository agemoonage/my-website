...
app.post("/save-html", async (req, res) => {
  ...
  const filePath = `public/archive/${safeFileName}`;
...
<body>
  ...
  try {
    // updateArchiveIndex();
    const url = await uploadToOSS(Buffer.from(htmlContent), filePath);
    res.json({
      message: `文件已保存为 ${safeFileName}`,
      path: `/public/archive/${safeFileName}`,
      url,
    });
  } catch (ossErr) {
